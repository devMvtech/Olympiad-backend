...
exports.createCourse = async (req, res, formattedFileUrls) => {
  ...
  try {
    ...
    // console.log(formattedFileUrls);cls
    const page = [];
    ...
    // Check if the title already exists in the maths table
    const titleExists = await db.query("SELECT * FROM maths WHERE title = $1", [
      title,
    ]);

    if (titleExists.rows.length > 0) {
      return res.status(400).json({
        error: "maths with the provided title already exist.",
      });
    }

    await db.query(
      `INSERT INTO maths (title, cover_img, page) VALUES ($1, $2, $3)`,
      [title, coverImg, page]
    );
...
exports.getallCourse = async (req, res) => {
  try {
    const { rows } = await db.query("select * from maths");
...
exports.getCourse = async (req, res) => {
  try {
    const { id } = req.params;
    const course = await db.query("SELECT * FROM maths WHERE id = $1", [id]);
...
exports.deleteCourse = async (req, res) => {
  try {
    const { id } = req.params;
    const course = await db.query("DELETE FROM maths WHERE id = $1", [id]);
